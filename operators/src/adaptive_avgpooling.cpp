    ...
    std::vector<std::shared_ptr<Tensor<float>>>& outputs) 
{
    ...
    {
        LOG(ERROR) << "The input and output tensor array size of the adaptive pooling layer do not match";
    ...
    for (uint32_t i = 0; i < batch; ++i) 
    {
        ...
        {
          LOG(ERROR) << "The input tensor array in the adaptive pooling layer has an empty tensor " << i << "th";
          return InferStatus::kInferFailedInputEmpty;
        }
        if (output_data != nullptr && !output_data->empty()) 
        {
            ...
            {
                LOG(ERROR) << "The output tensor array in the adaptive pooling layer has an incorrectly sized tensor " << i << "th";
    ...
    for (uint32_t i = 0; i < batch; ++i) 
    {
        ...
        CHECK(input_data != nullptr && !input_data->empty())
            << "The input tensor array in the adaptive pooling layer has an empty tensor "
            << i << "th";
        ...
        if (output_data == nullptr || output_data->empty()) 
        {
            DLOG(ERROR) << "The output tensor array in the adaptive pooling layer has an empty tensor "
                        << i << "th";
        ...
        CHECK(output_data->rows() == output_h_ && output_data->cols() == output_w_ && output_data->channels() == input_c)
            << "The output tensor array in the adaptive pooling layer has an incorrectly sized tensor "
            << i << "th";
    ...
    std::shared_ptr<Layer>& avg_layer) 
{
    ...
    auto output_hw = std::dynamic_pointer_cast<RuntimeParameterIntArray>(params.at("output_size"));
    if (!output_hw)
    {
...
LayerRegistererWrapper kAdaptiveAvgpoolingCreateInstance("nn.AdaptiveAvgPool2d", AdaptiveAveragePoolingLayer::CreateInstance);
