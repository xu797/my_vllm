    ...
    std::vector<std::shared_ptr<Tensor<float>>>& outputs) 
{
    ...
    const arma::fmat& weight_data_t = weight_data.t(); //weight转置
    ...
    for (uint32_t i = 0; i < batch; ++i) 
    {
        ...
        const uint32_t feature_dims = input_shapes.at(1); //rows
        const uint32_t in_features = input_shapes.at(2);  //cols
        ...
        CHECK(output->channels() == 1 && output->rows() == feature_dims && output->cols() == out_features_)
            << "The row of output tensor should be same to feature_dims_ and the col of output tensor should be same to output_features_ "
            << i << " th";
...
LayerRegistererWrapper kLinearCreateInstance("nn.Linear", LinearLayer::GetInstance);
