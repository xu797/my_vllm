...
ConvolutionLayer::ConvolutionLayer(
    uint32_t output_channel, uint32_t in_channel,
    ...
    std::vector<std::shared_ptr<Tensor<float>>>& outputs)
{
    ...
    {
        LOG(ERROR) << "The input and output tensor array size of the convolution layer do not match";
        return InferStatus::kInferFailedInputOutSizeMatchError;
    }

    if (weights_.empty()) 
    {
        LOG(ERROR) << "The number of kernel matrix in the convolution layer should be greater than zero";
    ...
    CHECK(kernel_h > 0 && kernel_w > 0 && kernel_c > 0)
        << "The size of kernel matrix in the convolution layer should be greater than zero";
    ...
    for (uint32_t i = 0; i < batch_size; ++i)
    {
        ...
        CHECK(col_len > 0) << "Output_h x output_w for the convolution layer should be greater than zero "
                        << i << " th";

        uint32_t input_c_group = input_c / groups_;
        CHECK(input_c_group == kernel_c) << "The number of channel for the kernel matrix and input tensor do not match";
        ...
        for (uint32_t g = 0; g < groups_; ++g)
        {
            ...
            CHECK(output_tensor->rows() == output_h &&
                ...
                << "The output tensor array in the convolution layer has an incorrectly sized tensor "
                << i << "th";
...
arma::fmat ConvolutionLayer::Im2Col(
    sftensor input, uint32_t kernel_w,
                ...
                for (uint32_t kw = 0; kw < kernel_w; ++kw) 
                {
                    ...
                    for (uint32_t kh = 0; kh < kernel_h; ++kh) 
                    {
                        if ((kh + r >= padding_h_ && kw + w >= padding_w_) &&
                            (kh + r < input_h + padding_h_ && kw + w < input_w + padding_w_)) 
                        {
...
void ConvolutionLayer::InitIm2ColWeight() 
{
    ...
    const uint32_t kernel_c = this->weights_.at(0)->channels();
    const uint32_t row_len = kernel_h * kernel_w;
    CHECK(kernel_h > 0 && kernel_w > 0 && kernel_c > 0) << "The size of kernel matrix should be greater than zero";
