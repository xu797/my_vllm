    ...
    std::vector<std::shared_ptr<Tensor<float>>>& outputs) 
{
    ...
    CHECK(!expressions.empty()) << "The expression parser failed to parse " << statement_;
    ...
    for (uint32_t i = 0; i < inputs.size(); ++i) 
    {
        ...
        {
            LOG(ERROR) << "The input tensor array in the expression layer has an empty tensor " << i << "th";
            return InferStatus::kInferFailedInputEmpty;
        }
    }
    ...
    for (uint32_t i = 0; i < batch_size; ++i)
    {
        ...
        {
            DLOG(ERROR) << "The output tensor array in the expression layer has an empty tensor " << i << "th";
            return InferStatus::kInferFailedOutputEmpty;
        }
    ...
    for (const auto& token_node : token_nodes)
    {
        ...
        else
        {
            ...
            CHECK(input_node2.size() == batch_size)
                << "The second operand doesn't have appropriate number of tensors, which need "
                << batch_size;
            op_stack.pop();
