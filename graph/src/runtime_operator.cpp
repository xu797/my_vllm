    ...
    const std::vector<std::shared_ptr<RuntimeOperator>>& operators)
{
    ...
    for(uint32_t i = 0; i < pnnx_operators.size(); ++i)
    {
        ...
        pnnx::Operand *operand = operands.front(); //operand = operands[0]
            ...
            for (uint32_t b = 0; b < batch; ++b) 
            {
                ...
                else 
                {
                    ...
                    if (tensor_shapes.at(0) != 1 ||
                        ...
                        {
                            DLOG(WARNING) << "The shape of tensor do not adapting with output operand";
                            const auto &target_shapes = std::vector<uint32_t>{(uint32_t) operand_shapes.at(1), (uint32_t) operand_shapes.at(2)};
                            output_tensor->Reshape(target_shapes);
                        }
                }
            }
